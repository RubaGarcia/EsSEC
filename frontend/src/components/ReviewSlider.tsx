...
export default function ReviewSlider({ blurb, reviewer }: ReviewSliderProps) {
  ...
  const showPreviousReview = () => {
    const previousIndex =
      currentIndex === 0 ? reviewer.length - 1 : currentIndex - 1;
    setCurrentIndex(previousIndex);
  };

  const showNextReview = () => {
    const nextIndex =
      currentIndex === reviewer.length - 1 ? 0 : currentIndex + 1;
  ...
  return (
    <section className="bg-white dark:bg-gray-900">
      <div className="container px-6 py-10 mx-auto space-x-5 space-y-10">
          ...
          <div className="flex flex-col items-center flex-grow mx-4">
            ...
            <div className="flex flex-col items-center justify-center mt-8">
              <img
                ...
                src={
                  currentReview.fields?.image?.fields?.asset?.fields?.file
                    ?.url ||
